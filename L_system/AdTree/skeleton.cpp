...
Skeleton::Skeleton()
        : Points_(nullptr), KDtree_(nullptr), quiet_(true) {
    TrunkRadius_ = 0;
    TreeHeight_ = 0;
    BoundingDistance_ = 0;
    VecLeaves_.clear();
    ...
    for (long nTet = 0; nTet < tet_out.numberoftetrahedra; nTet++) {
        ...
        for (long i = tet_first; i < tet_first + tet_out.numberofcorners; i++)
            for (long j = i + 1; j < tet_first + tet_out.numberofcorners; j++)
                add_edge(vertex(tet_out.tetrahedronlist[i], delaunay_), vertex(tet_out.tetrahedronlist[j], delaunay_),
                         delaunay_);
    }
            ...
            for (std::size_t n = 0; n < numOfSlicesCurrent[numOfSlicesCurrent.size() - 1]; ++n) {
                ...
                if (n == 0) {
                    ...
                    if (distance2(prev, point) >
                        epsilon<float>() * 10) { // in case of duplicated points (tiny cylinder)
                        interpolatedPoints.push_back(point);
                        interpolatedRadii.push_back(sourceRadius - n * deltaOfRadius);
                    }
                }
    ...
    while (true) {
        ...
        for (SGraphAdjacencyIterator aIter = aj.first; aIter != aj.second; ++aIter) {
            if (*aIter != (*i_Graph)[currentV].nParent) {
                ...
                double subtreeRatio =
                        ((*i_Graph)[*aIter].lengthOfSubtree + child2Current) / (*i_Graph)[currentV].lengthOfSubtree;
        ...
        for (SGraphVertexIterator cIter = vp.first; cIter != vp.second; ++cIter) {
            ...
            if ((out_degree(dVertex, simplified_skeleton_) > 2) ||
                ((simplified_skeleton_[dVertex].nParent == dVertex) &&
                 (out_degree(dVertex, simplified_skeleton_) > 1))) {
...
bool Skeleton::merge_vertices(Graph *i_Graph, SGraphVertexDescriptor i_dSource, SGraphVertexDescriptor i_dTarget,
                              double i_wSource, double i_wTarget) {
    //initialize
    ...
    for (std::set<SGraphVertexDescriptor>::const_iterator cIter = sGroupVertices.begin();
         cIter != sGroupVertices.end(); ++cIter) {
        double mapRadius;
    ...
    if (pV.lengthOfSubtree == 0)
        ...
        pNew = i_wSource * pSource * (*i_Graph)[i_dSource].lengthOfSubtree +
               i_wTarget * pTarget * (*i_Graph)[i_dTarget].lengthOfSubtree;
        pNew = pNew /
               (i_wSource * (*i_Graph)[i_dSource].lengthOfSubtree + i_wTarget * (*i_Graph)[i_dTarget].lengthOfSubtree);
    }
    pV.cVert = pNew;
...
double
Skeleton::compute_merge_value(Graph *i_Graph, SGraphVertexDescriptor i_dSource, SGraphVertexDescriptor i_dTarget) {
...
void Skeleton::assign_points_to_edges() {
    ...
    for (SGraphEdgeIterator eIter = ep.first; eIter != ep.second; ++eIter) {
        ...
        if (source(currentE, simplified_skeleton_) ==
            simplified_skeleton_[target(currentE, simplified_skeleton_)].nParent) {
        ...
        Vector3D pSource(simplified_skeleton_[sourceV].cVert.x, simplified_skeleton_[sourceV].cVert.y,
                         simplified_skeleton_[sourceV].cVert.z);
        Vector3D pTarget(simplified_skeleton_[targetV].cVert.x, simplified_skeleton_[targetV].cVert.y,
                         simplified_skeleton_[targetV].cVert.z);
...
void Skeleton::get_graph_for_smooth(std::vector<Path> &pathList) {
    ...
    while (cursor < pathList.size()) {
        ...
        if ((out_degree(endV, simplified_skeleton_) == 1) && (endV != simplified_skeleton_[endV].nParent))
            ...
            std::pair<SGraphAdjacencyIterator, SGraphAdjacencyIterator> adjacencies = adjacent_vertices(endV,
                                                                                                        simplified_skeleton_);
